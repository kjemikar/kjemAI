...
for key in resultat2.oppgaver:
    ...
    oppgave.testresultat_fleirval_enkel["gemini-1.0-pro-vision-001"] = oppgave.testresultat_fleirval_enkel["gemini-1.0-pro-vision-001"].strip().capitalize()

#validate results
for key in resultat2.oppgaver:
    oppgave = resultat2.oppgaver[key]
    if not (oppgave.testresultat_fleirval_enkel["gpt-4-turbo"] in ["A", "B", "C", "D"]):
        print(f"Feil svar GPT-4: {oppgave.testresultat_fleirval_enkel['gpt-4-turbo']}")
    if not (oppgave.testresultat_fleirval_enkel["gemini-1.0-pro-vision-001"] in ["A", "B", "C", "D"]):
        print(f"Feil svar Gemini-1.0: {oppgave.testresultat_fleirval_enkel['gemini-1.0-pro-vision-001']}")
    if not (oppgave.testresultat_fleirval_enkel["gemini-1.5-pro-preview-0409"] in ["A", "B", "C", "D"]):
        print(f"Feil svar Gemini-1.5: {oppgave.testresultat_fleirval_enkel['gemini-1.5-pro-preview-0409']}")

for key in resultat2.oppgaver:
    oppgave = resultat2.oppgaver[key]
    antal_oppgaver += 1
    if oppgave.fasit == oppgave.testresultat_fleirval_enkel["gpt-4-turbo"]:
        riktige_svar_gpt4 += 1
    if oppgave.fasit == oppgave.testresultat_fleirval_enkel["gemini-1.0-pro-vision-001"]:
        riktige_svar_gemini10 += 1
    if oppgave.fasit == oppgave.testresultat_fleirval_enkel["gemini-1.5-pro-preview-0409"]:
